...
approval, and pre-release validation gate. The gate takes `changeRoot` from
status and uses `<changeRoot>/release.md` and
`<changeRoot>/verification.md`, refuses to archive without a successful
recorded release and exact approval, and re-runs strict OpenSpec validation
after an external action. The bootstrap does not add another archive command;
it relies on the installed CLI and the adapted prompt's gate so an unapproved
or unverified change cannot archive. The archive prompt steps are titled
"Enforce framework release and approval gates before archive" and require
...
- The generated `/opsx-archive` prompt now enforces a release, approval, and
  pre-release validation gate through the reviewed SDD adaptation. Its evidence
  paths are fixed under the `changeRoot` returned by status; a project that
  deliberately changes this convention owns and must revalidate that custom
  gate.
