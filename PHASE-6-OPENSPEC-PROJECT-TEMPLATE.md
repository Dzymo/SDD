...
| `core` lacks the continuation surface named by generated apply guidance | `opsx-apply.md` instructs `/opsx-continue` for a blocked change. A fresh `blocked-apply` change returned `state: "blocked"` and named the unavailable `openspec-continue-change` skill. The core bootstrap contains only apply, archive, explore, propose, and sync. | Do not introduce a custom command. Create missing artifacts through `openspec status` and `openspec instructions`, then retry apply. Treat this as an upstream core-profile limitation. |
| Generated archive prompt is not PowerShell-idempotent | In the fresh Windows project, its `openspec\changes\archive` directory already existed. The generated `/opsx-archive` `mkdir -p` step emitted PowerShell's `ResourceExists` error. The reviewed `@fission-ai/openspec@1.5.0` CLI exposes `openspec archive [change-name]` directly. | On Windows, archive a complete change through `openspec archive <change-name>` rather than the generated `/opsx-archive` prompt until OpenSpec corrects its generated command. |
