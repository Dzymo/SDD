...
OpenCode configuration change, a custom OpenSpec schema, or an additional
command outside the five core command names.
...
| `templates\project\.opencode\commands\opsx-*.md` | Five reviewed adaptations of the CLI-generated OpenCode core command surface: apply, archive, explore, propose, and sync. They provide session-first interviews, agent-maintained artifacts, next-step advice, and framework gates. |
...
| Template command surface cannot drift locally | `scripts\Test-FrameworkSkeleton.ps1`; copied bootstrap preflight | Established. The gates require exactly five OpenSpec command files, reject additional project-template commands, and require the session-first/next-step contract. |
...
| Generated archive is not PowerShell-idempotent and is not gated by release, approval, or pre-release validation | In the fresh Windows project, its `openspec\changes\archive` directory already existed. The generated `/opsx-archive` `mkdir -p` step emitted PowerShell's `ResourceExists` error. The default prompt also lacked any release, user-approval, or strict pre-release validation gate, so an unapproved or unverified change could archive. | The adapted prompt uses one of four portable installed-CLI forms: local/store multiplied by synced/unsynced. Synced changes use `--skip-specs --yes`; unsynced changes use `--yes`. It resolves evidence under the `changeRoot`, chooses release-applicable or genuine no-external-release gates from actual scope, and fails closed on missing requirement coverage, focused validation, approval/result when applicable, post-release smoke when applicable, or fresh strict validation. |
...
adapted core command files, including the session-first artifact ownership,
next-command advice, adaptive interview, and OpenChamber mode-advice contract.
...
The adapted `/opsx-archive` prompt now enforces the framework release,
approval, verification, and pre-archive validation gates. The gate takes
`changeRoot` from status and uses `<changeRoot>/verification.md` plus
`<changeRoot>/release.md` only when release is applicable. It supports local or
store roots and applies the delta exactly once by selecting `--skip-specs --yes`
after sync or `--yes` before sync. A release-applicable change requires exact
approval, successful external result, post-release smoke, and fresh strict
validation. A genuine no-external-release change requires completed tasks and
artifacts, full requirement coverage, fresh focused validation, strict
validation, and `releaseApplicable: false` with a reason. The bootstrap does
not add another archive command; it relies on the installed CLI and the adapted
prompt so neither branch can bypass its applicable gates.

Compatibility marker: the `/opsx-archive` prompt now enforces a release, approval, and
pre-release validation gate for the release-applicable branch; this statement
does not impose a release on a genuine no-external-release change.
...
- The generated `/opsx-archive` prompt now enforces release-applicable and
  no-external-release branches through the reviewed SDD adaptation. Its
  evidence paths are fixed under the `changeRoot` returned by status; a project
  that deliberately changes this convention owns and must revalidate that
  custom gate.
