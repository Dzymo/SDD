...
Assert-Contains -Text $archiveCmd -Expected 'Release-applicable branch' -Message 'opsx-archive must include the release-applicable branch.'
Assert-Contains -Text $archiveCmd -Expected 'No-external-release branch' -Message 'opsx-archive must include the no-external-release branch.'
Assert-Contains -Text $archiveCmd -Expected 'NO-RELEASE-GUARD' -Message 'opsx-archive must advertise the no-release guard contract marker.'
Assert-Contains -Text $archiveCmd -Expected 'RELEASE-GATE-BRANCH' -Message 'opsx-archive must advertise the release gate branch contract marker.'
Assert-Contains -Text $archiveCmd -Expected 'releaseApplicable' -Message 'opsx-archive must require a releaseApplicable decision in evidence.'
Assert-Contains -Text $archiveCmd -Expected 'Pre-release / pre-archive strict validation gate' -Message 'opsx-archive must include a pre-release/pre-archive strict validation gate.'
Assert-Contains -Text $archiveCmd -Expected 'Verification gate' -Message 'opsx-archive must include a verification gate.'
Assert-True -Condition (-not $archiveCmd.Contains('OpenSpec archive is forbidden without a successful recorded release')) -Message 'opsx-archive must not regress to release-required-for-every-change.'
Assert-True -Condition (-not ($archiveCmd -match 'for every change')) -Message 'opsx-archive must not regress to phrasing that mandates a release for every change.'
Assert-True -Condition (-not ($archiveCmd -match 'require.{0,40}release.{0,40}(every|all|any)')) -Message 'opsx-archive must not regress to blanket all-changes-must-have-release phrasing.'
Assert-True -Condition (-not ($archiveCmd -match 'must be a release')) -Message 'opsx-archive must not regress to "every change must be a release" phrasing.'
Assert-True -Condition (-not $archiveCmd.Contains('every change must perform a release')) -Message 'opsx-archive must not regress to the "every change must perform a release" phrasing.'
...
Assert-Contains -Text $syncCmd -Expected '--store <id>' -Message 'opsx-sync must reference the selected store argument.'
Assert-Contains -Text $syncCmd -Expected 'cùng một `--store <id>`' -Message 'opsx-sync must pin the same store argument on every lifecycle command.'
